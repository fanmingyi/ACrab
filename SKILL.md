...
- 支持 4 种后端：`local`（本地 mlx-vlm）、`api`（云端 API）、`vllm`（局域网 vLLM，0-1000 归一化坐标）、`remote`（宿主机 HTTP 服务）
- `debug=true` 时调试图保存到 `$SKILL_TMP/debug_visual_locator.png`
- 不存在时使用默认值（debug=false，backend=local）
...
**元素树找不到时 → 子 Agent 视觉定位**：
启动 Agent 工具，让子 Agent 调用 `visual_locator.py`，传入**详细的元素描述**和屏幕分辨率。

描述要求（必须包含足够上下文，帮助 VLM 精确定位）：
- **外观**：颜色、形状、大小
- **文字内容**：按钮/标签上的文字
- **位置上下文**：在什么区域、什么元素的上方/下方/旁边

描述示例：
- ✅ `"灰色的圆角按钮包含'下一步'文字，在屏幕下方两个输入框下面"`
- ✅ `"蓝色的Wi-Fi图标开关，位于页面上方第一个设置选项的右侧"`
- ✅ `"悬浮键盘上的白色'确定'按钮，在键盘右下角"`
- ❌ `"下一步"` （太简略，容易定位错误）

子 Agent 执行流程（注意：必须传入 `--screen-size`）：
```bash
python {SKILL_DIR}/visual_locator.py $SKILL_TMP/mobile_screenshot.png "详细元素描述" --screen-size {宽}x{高}
```

脚本自动完成图片预处理、VLM 推理、坐标映射（VLM 坐标 → 屏幕坐标）。
子 Agent 返回 JSON 结果，包含已映射的 bbox（x1, y1, x2, y2）和 center。

**主对话收到 bbox 后**，坐标已自动映射完成，直接用中心点进行点击：`cx=center[0], cy=center[1]`。
...
### 第2轮（2个并行 Bash 调用）
依赖第1轮结果，同时发起以下 2 个 Bash 工具调用：
- **调用1**：`adb -s {dev} shell ime enable com.android.adbkeyboard/.AdbIME && adb -s {dev} shell ime set com.android.adbkeyboard/.AdbIME && adb -s {dev} logcat -c` — 启用键盘 + 清除日志
- **调用2**：`python {SKILL_DIR}/visual_locator.py --selfcheck` — 视觉定位自检（最耗时）
  - 根据 config.properties 中的 `backend` 配置（local/api）检查对应依赖
...
| 清除日志 | `adb -s {dev} logcat -c` |
| dump全量日志 | `adb -s {dev} logcat -d > $SKILL_TMP/mobile_crash_report/{时间戳}/logcat.txt` |
...
可选参数：`--debug`、`--no-debug`、`--model MODEL`、`--backend local|api|vllm|remote`
...
**核心原则：优先查阅用户在意图分析阶段（7.3）确认的异常处理策略。** 如果当前遇到的异常已有用户预设的处理方式，严格按照用户的选择执行（暂停交给用户 / 中断输出报告 / 自动处理）。以下为策略未覆盖时的默认行为：

- **C（无变化）**→ 换定位方式重试。连续 2 次点击无变化→先滑动屏幕再重新定位点击
- **B（走错页面）**→ `adb shell input keyevent KEYCODE_BACK` 返回，重新观察后操作
- **弹窗/权限/广告/更新提示** → **优先处理弹窗**（点击"关闭"/"允许"/"跳过"/"同意"），再继续任务。打开应用后的第一步必须先检查有无弹窗。**注意**：如果用户在 7.3 中对权限弹窗选择了"全部拒绝"或"逐个确认"，遵循用户选择
- **人机验证/验证码** → 如果用户在 7.3 中选择了"暂停交给我"，则停下来通知用户手动操作；如果选择了"中断"，则执行规则九输出结论报告
...
- 循环处理直到无弹窗，再继续主任务
- 权限弹窗默认行为参照用户在 7.3 中的选择（未指定则默认"允许"）
...
# 3. dump 全量日志（不过滤，任务期间全部日志）
adb -s {dev} logcat -d > $SKILL_TMP/mobile_crash_report/{时间戳}/logcat.txt
```
...
### 前提
环境自检阶段已执行 `adb -s {dev} logcat -c` 清空缓冲区，所以 `logcat -d` dump 出的是本次任务运行期间的全部日志。

## 规则九：中断与结论报告

当执行过程中触发了用户选择的「中断流程」策略，或遇到无法继续的阻塞问题时，**立即停止操作**，输出结论报告：
